...
describe('rxjs', () => {
  ...
  it('use Observable.of to emit A, B, C', done => {
    ...
    const ob2b$ = Observable.of('A', 'B', 'C');
  ...
  it('make err obs but catch and emit obj instead', done => {
    ...
    // object instead
    const ob3b$ = Observable.throw(new Error('error 3b'))
  ...
  it('every 0.5s use mergeMap to fetch ajax, only twice', done => {
    ...
    const fetch$ = Observable.ajax.getJSON('/fake-api.json')
                             .map(data => data.items)
                             .map(items => ({
                               type: 'FETCH_SUCCESS',
                               payload: items
    ...
    const ob7a$ = Observable.interval(500)
                            .take(2)
                            .mergeMap(x => fetch$);
  ...
  it('like previous but throttle 1/s', done => {
    ...
    const ob7b$ = Observable.interval(500)
                            .take(2)
                            .throttleTime(1000)
                            .mergeMap(x => fetch$);
  ...
  it('like 7a but debounce for 0.6s', done => {
    ...
    const ob7c$ = Observable.interval(500)
                            .take(2)
                            .debounceTime(600)
                            .mergeMap(x => fetch$);
  ...
  it('send and receive from reconnecting websocket', done => {
    ...
    wsConnected$
      .filter(x => x === false) // only false, closes
