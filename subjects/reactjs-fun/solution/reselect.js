...
describe('reactjs-fun/reselect', () => {
  ...
  const mergedWidgetSelector = createSelector(
    ...
    /* complex computation */
    (widgets, styles, brands) => {
      const findStyle = styleId => find(s => s.styleId === styleId)(styles);
      const findBrand = brandId => find(b => b.brandId === brandId)(brands);
      const mergedWidgets = widgets.map(w => {
        const style = findStyle(w.styleId);
        const brand = findBrand(w.brandId);
        /* create new widget with style and brand included */
        return compose(
          set('style', style),
          set('brand', brand),
          unset('styleId'),
