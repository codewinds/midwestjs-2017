...
describe('reactjs-fun/recompose', () => {
  ...
  it('should handle form state and provide handlers', done => {
    ...
    let lastSubmitValues;
    const Comp = compose(
      withState('values', 'updateValues', { itemName: '', desc: ''}),
      withHandlers({
  ...
  it('should be able to fetch data on mount and update when loaded', done => {
    ...
    const DynamicContent = compose(
      lifecycle({
        componentDidMount() {
          // fetch the data here
          fetchPromise = axios.get('/fake-api.json')
            .then(resp => resp.data.items) // use items property of payload
            .then(items => this.setState({ items }));
        }
      }),
      branch(props => !props.items, renderComponent(Loading))
    )(MainContent);
